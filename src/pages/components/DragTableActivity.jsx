...
export const DragTableActivity = () => {
  ...
  const sumarValoresPorPeriodo = () => {
    ...
    if (Array.isArray(activityVectors)) {
      activityVectors.forEach((item) => {
  ...
  const suma_leakage = newData
    .map((n) => {
      const totalValue = Array.isArray(activityVectors)
        ? activityVectors.reduce((acc, v) => {
      ...
      <Grid>
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {Array.isArray(activityVectors) &&
              activityVectors.length !== 0 ? (
                activityVectors.map((item) => (
